...
Claude Code/Codex/opencode などのCLI型AIコーディングエージェント(以降コーディングエージェント)の普及に伴い、tmux上でエージェントを管理するラッパーツールが雨後の筍のように登場しています。ご多分に漏れず自分も作ってしまいました。既存のツールはTUI形式が多いのですが、ターミナルの貴重な画面領域を専有されるのが自分には合わず、macOSのメニューバーからGUIをトグル表示する方式を採りました。もちろんショートカットで完結するのでマウス操作は不要です。

動作の様子は以下で、リポジトリのフォルダアイコンの中に、コーディングエージェントが動作中のtmuxペインを閲覧、選択して遷移できます。ショートカットで全て飛べますのでマウス操作は必要ないようにしています。マウスを使わずmacOSから数キーでレスポンス待ちのAIエージェントに飛べるのが一番よく使う機能です。
